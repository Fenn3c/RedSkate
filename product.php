...
$product = $db->getProductById($_GET['id_product']);
$category = $db->getCategoryById($product['id_category']);
$similarProducts = $db->getSimilarProducts($_GET['id_product']);
$productReviews = $db->getProductReviews($_GET['id_product']);
$productImages = $db->getProductImages($_GET['id_product']);
?>
<!DOCTYPE html>
<html lang="ru">
...
<head>
    ...
    <title><?= $product['name'] ?> - RedSkate</title>
        ...
        <div class="main__wrap">
            ...
            <section class="product">
                <div class="product__pictures">
                    <img src="./data/product-gallery/<?= $productImages[0]['image'] ?>" alt="" class="product__main-img" id="main-product-img">
                    ...
                    <div class="product__gallery">
                        ...
                        <div class="product__gallery-slider" id="slider">
                            <? foreach ($productImages as $index => $productImage) : ?>
                                <img src="./data/product-gallery/<?= $productImage['image'] ?>" alt="" class="product__gallery-img <?= $index == 0 ? 'product__gallery-img_active' : '' ?>">
                            <? endforeach; ?>
                        </div>
                ...
                <div class="products__list">
                    <? foreach ($similarProducts as $similarProduct) : ?>
                        <div class="products__list-item product-card">
                            <div class="product-card__bg" style="background-color: <?= $similarProduct['color'] ?>">

                                <? if ($similarProduct['old_price']) : ?>
                                    <p class="product-card__discount"><?= get_discount($similarProduct['price'], $similarProduct['old_price']) ?></p>
                                <? endif; ?>
                                <a href="./product.php?id_product=<?= $similarProduct['id_product'] ?>" class="product-card__link">
                                    <img src="./data/product-preview/<?= $similarProduct['preview'] ?>" alt="" class="product-card__img">
                                </a>
                            </div>

                            <a href="#" class="product-card__link">
                                <p class="product-card__title"><?= $similarProduct['name'] ?></p>
                            </a>

                            <?
                            $rating = $db->getProductRating($similarProduct['id_product']);
                            ?>
                            ...
                            <div class="product-card__rating rating">
                                ...
                                <? for ($i = 0; $i < 5 - ceil($rating); $i++) : ?>
                                    <span class="rating__star__empty"></span>
                                <? endfor ?>
                                <span class="rating__count"><?= $db->getProductRatingCount($similarProduct['id_product']) ?></span>
                            </div>

                            <div class="product-card__bottom">
                                <span class="product-card__price"><?= format_price($similarProduct['price']) ?></span>
                                <?
                                if ($similarProduct['old_price']) :
                                ?>
                                    <span class="product-card__price_crossed"><?= format_price($similarProduct['old_price']) ?></span>
                                <? endif; ?>
                                <div class="product-card__actions">
                                    <form action="./actions/add_to_favourites.php" method="post" class="">
                                        ...
                                        $isInCart = false;
                                        if ($id_user) {
                                            $isInFavourite = $db->isInFavourites($id_user, $similarProduct['id_product']);
                                            $isInCart = $db->isInCart($id_user, $similarProduct['id_product']);
                                        }
                                        ?>
                                        <input type="hidden" value="<?= $similarProduct['id_product'] ?>" name="id_product">
                                        <button class="product-card__action <?= $isInFavourite ? 'product-card__action_active' : '' ?>" type="submit">
                                            <svg width="24" height="22" viewBox="0 0 24 22" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                <mask id="mask0_48_1274" style="mask-type:luminance" maskUnits="userSpaceOnUse" x="0" y="0" width="24" height="22">
                                                    <path d="M7.5 2C4.4625 2 2 4.4625 2 7.5C2 13 8.5 18 12 19.163C15.5 18 22 13 22 7.5C22 4.4625 19.5375 2 16.5 2C14.64 2 12.995 2.9235 12 4.337C11.4928 3.6146 10.8191 3.02505 10.0358 2.61824C9.25245 2.21144 8.38265 1.99938 7.5 2Z" fill="white" />
            ...
            <section class="reviews">
                <div class="reviews__title-wrap">
                    ...
                    <span class="reviews__count"><?= count($productReviews) ?></span>
                </div>
                <?php
                $can_review = $db->canReview($sessionManager->idUser(), $product['id_product']);
                $has_review = $db->hasReview($sessionManager->idUser(), $product['id_product']);
                if ($can_review && !$has_review) :
                ?>
                    <form class="reviews__send" action="./actions/create_review.php" method="post" id="review-form">
                        ...
                        <div class="reviews__send-right">
                            <div class=" rating rating_input">
                                ...
                                <input name="rating" type="radio" value="1" id="rating-radio-0" class="rating__input_input" checked></input>
                                <label for="rating-radio-1" class="rating__label_input">
                                </label>

                                <input name="rating" type="radio" value="2" id="rating-radio-1" class="rating__input_input"></input>
                                <label for="rating-radio-2" class="rating__label_input">
                                </label>

                                <input name="rating" type="radio" value="3" id="rating-radio-2" class="rating__input_input"></input>
                                <label for="rating-radio-3" class="rating__label_input">
                                </label>

                                <input name="rating" type="radio" value="4" id="rating-radio-3" class="rating__input_input"></input>
                                <label for="rating-radio-4" class="rating__label_input">
                                </label>

                                <input name="rating" type="radio" value="5" id="rating-radio-4" class="rating__input_input"></input>
                            </div>
                    ...
                    <? foreach ($productReviews as $review) : ?>
                        <div class="reviews__review review">
                            <div class="review__left">
                                <?php
                                $pfp =  $review['pfp'] ? './data/pfps/' . $review['pfp'] : './assets/img/png/user-default.png';
                                ?>
                                <img src="<?= $pfp ?>" alt="" class="review__user-img">
                                <div class="review__content">
                                    <p class="review__user-name"><?= $review['name'] ?></p>

                                    <?
                                    $rating = $db->getReviewRating($review['id_review']);
                                    ?>
                                    <div class="rating">
                                        <? for ($i = 0; $i < (int)$rating; $i++) : ?>
                                            <span class="rating__star"></span>
                            ...
                            <time class="review__time"> <?= $review['time'] ?></time>
                        </div>
                    <? endforeach; ?>
